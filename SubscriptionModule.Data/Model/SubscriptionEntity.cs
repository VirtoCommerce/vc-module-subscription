...
namespace SubscriptionModule.Data.Model
{
    public class SubscriptionEntity : AuditableEntity
    {
        ...
        {
            OrderRelations = new NullCollection<SubscriptionOrderRelationEntity>();
        }
        ...
        public ObservableCollection<SubscriptionOrderRelationEntity> OrderRelations { get; set; }
        ...
        public virtual Subscription ToModel(Subscription subscription)
        {
            ...
            subscription.CustomerOrdersIds = this.OrderRelations.Select(x => x.CustomerOrderId).ToArray();     
        ...
        {          
            ...
            if(subscription.CustomerOrdersIds != null)
            {
                this.OrderRelations = new ObservableCollection<SubscriptionOrderRelationEntity>(subscription.CustomerOrdersIds.Select(x => new SubscriptionOrderRelationEntity { CustomerOrderId = x }));
            }
            else if(subscription.CustomerOrders != null)
            {
                this.OrderRelations = new ObservableCollection<SubscriptionOrderRelationEntity>(subscription.CustomerOrders.Select(x => new SubscriptionOrderRelationEntity { CustomerOrderId = x.Id }));
            }
        ...
        public virtual void Patch(SubscriptionEntity target)
        {
            ...
            if (!this.OrderRelations.IsNullCollection())
            {
                var orderRelationComparer = AnonymousComparer.Create((SubscriptionOrderRelationEntity x) => x.CustomerOrderId);
                this.OrderRelations.Patch(target.OrderRelations, orderRelationComparer, (sourceRelation, targetRelation) => { return; });
            }
        }
